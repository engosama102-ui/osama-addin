...
async function matchSize(type) {
  await PowerPoint.run(async (context) => {
    const sel = context.presentation.getSelectedShapes();
    sel.load("items/width,items/height");
    await context.sync();
    const items = sel.items;
    if (items.length < 2) return showStatus("Select at least 2","err");
...
async function convertToRoundRect() {
  await PowerPoint.run(async (context) => {
    const sel = context.presentation.getSelectedShapes();
    sel.load("items/left,items/top,items/width,items/height,items/fill/foregroundColor,items/lineFormat/color,items/lineFormat/weight,items/lineFormat/visible");
    await context.sync();

    if (!sel.items.length) return showStatus("No shapes selected","err");
    const slide = context.presentation.getSelectedSlides().getItemAt(0);
    let count=0;

    for (const shape of sel.items) {
      const L=shape.left, T=shape.top, W=shape.width, H=shape.height;
      let fill="#4472C4", lc="#000", lw=1, lv=false;
      ...
      ns.lineFormat.color=lc; ns.lineFormat.weight=lw; ns.lineFormat.visible=lv;
      await context.sync();
      count++;
    }
...
// ── CORNER RADIUS ─────────────────────────────────────
// Formula from VBA research:
// Adjustments(1) = radiusPt / shortSide
// where shortSide = min(width, height)
// Range: 0 to 0.5 (max = shortSide/2)
  ...
  await PowerPoint.run(async (context) => {
    ...
    for (const shape of sel.items) {
      try {
        ...
        // adjValue = radiusPt / shortSide, max 0.5
    ...
    if (applied > 0) showStatus(`✓ Radius ${radiusPt}pt applied to ${applied} shape(s)`);
...
async function applyOpacity(value) {
  await PowerPoint.run(async (context) => {
    ...
    sel.load("items/type");
    await context.sync();
    sel.items.forEach(s => { try { s.fill.transparency=1-parseFloat(value)/100; } catch(e){} });
    await context.sync();
  }).catch(()=>{});
}
...
async function applyFillColor(hex) {
  await PowerPoint.run(async (context) => {
    ...
    sel.load("items/type");
    await context.sync();
    sel.items.forEach(s => { try { s.fill.setSolidColor(hex); } catch(e){} });
    await context.sync();
  }).catch(()=>{});
}
...
async function applyBorderColor(hex) {
  await PowerPoint.run(async (context) => {
    ...
    sel.load("items/type");
    await context.sync();
    sel.items.forEach(s => { try { s.lineFormat.color=hex; s.lineFormat.visible=true; } catch(e){} });
    await context.sync();
  }).catch(()=>{});
}
...
const STANDARD_SHAPES = [
  { name:"Rectangle",     type:PowerPoint.GeometricShapeType.rectangle,           svg:'<rect x="3" y="5" width="14" height="10" rx="0" fill="#555"/>' },
  { name:"RoundRect",     type:PowerPoint.GeometricShapeType.roundRectangle,      svg:'<rect x="3" y="5" width="14" height="10" rx="3" fill="#555"/>' },
  { name:"Ellipse",       type:PowerPoint.GeometricShapeType.ellipse,             svg:'<ellipse cx="10" cy="10" rx="8" ry="6" fill="#555"/>' },
  { name:"Triangle",      type:PowerPoint.GeometricShapeType.isoscelesTriangle,   svg:'<polygon points="10,3 18,17 2,17" fill="#555"/>' },
  { name:"RtTriangle",    type:PowerPoint.GeometricShapeType.rightTriangle,       svg:'<polygon points="2,17 18,17 2,3" fill="#555"/>' },
  { name:"Diamond",       type:PowerPoint.GeometricShapeType.diamond,             svg:'<polygon points="10,2 18,10 10,18 2,10" fill="#555"/>' },
  { name:"Pentagon",      type:PowerPoint.GeometricShapeType.regularPentagon,     svg:'<polygon points="10,2 18,8 15,18 5,18 2,8" fill="#555"/>' },
  { name:"Hexagon",       type:PowerPoint.GeometricShapeType.hexagon,             svg:'<polygon points="5,2 15,2 20,10 15,18 5,18 0,10" fill="#555"/>' },
  { name:"Octagon",       type:PowerPoint.GeometricShapeType.octagon,             svg:'<polygon points="6,2 14,2 18,6 18,14 14,18 6,18 2,14 2,6" fill="#555"/>' },
  { name:"Parallelogram", type:PowerPoint.GeometricShapeType.parallelogram,       svg:'<polygon points="5,17 2,3 15,3 18,17" fill="#555"/>' },
  { name:"Trapezoid",     type:PowerPoint.GeometricShapeType.trapezoid,           svg:'<polygon points="3,17 17,17 14,3 6,3" fill="#555"/>' },
  { name:"Star4",         type:PowerPoint.GeometricShapeType.star4,               svg:'<polygon points="10,2 12,8 18,10 12,12 10,18 8,12 2,10 8,8" fill="#555"/>' },
  { name:"Star5",         type:PowerPoint.GeometricShapeType.star5,               svg:'<polygon points="10,2 12,7 18,7 13,11 15,17 10,13 5,17 7,11 2,7 8,7" fill="#555"/>' },
  { name:"Star6",         type:PowerPoint.GeometricShapeType.star6,               svg:'<polygon points="10,2 12,6 16,4 14,8 18,10 14,12 16,16 12,14 10,18 8,14 4,16 6,12 2,10 6,8 4,4 8,6" fill="#555"/>' },
  { name:"Arrow R",       type:PowerPoint.GeometricShapeType.rightArrow,          svg:'<polygon points="2,7 13,7 13,4 18,10 13,16 13,13 2,13" fill="#555"/>' },
  { name:"Arrow L",       type:PowerPoint.GeometricShapeType.leftArrow,           svg:'<polygon points="18,7 7,7 7,4 2,10 7,16 7,13 18,13" fill="#555"/>' },
  { name:"Arrow Up",      type:PowerPoint.GeometricShapeType.upArrow,             svg:'<polygon points="10,2 16,8 13,8 13,18 7,18 7,8 4,8" fill="#555"/>' },
  { name:"Arrow Dn",      type:PowerPoint.GeometricShapeType.downArrow,           svg:'<polygon points="10,18 4,12 7,12 7,2 13,2 13,12 16,12" fill="#555"/>' },
  { name:"Chevron",       type:PowerPoint.GeometricShapeType.chevron,             svg:'<polygon points="2,5 13,5 18,10 13,15 2,15 7,10" fill="#555"/>' },
  { name:"Callout",       type:PowerPoint.GeometricShapeType.wedgeRectCallout,    svg:'<rect x="2" y="2" width="14" height="10" rx="1" fill="#555"/><polygon points="6,12 4,18 10,12" fill="#555"/>' },
  { name:"RndCallout",    type:PowerPoint.GeometricShapeType.wedgeRoundRectCallout, svg:'<rect x="2" y="2" width="14" height="10" rx="3" fill="#555"/><polygon points="6,12 4,18 10,12" fill="#555"/>' },
  { name:"Cylinder",      type:PowerPoint.GeometricShapeType.can,                 svg:'<rect x="4" y="6" width="12" height="10" fill="#555"/><ellipse cx="10" cy="6" rx="6" ry="2" fill="#777"/>' },
  { name:"Cube",          type:PowerPoint.GeometricShapeType.cube,                svg:'<rect x="4" y="7" width="10" height="10" fill="#555"/><polygon points="4,7 8,3 18,3 18,13 14,17 14,7" fill="#777"/><polygon points="4,7 14,7 18,3 8,3" fill="#999"/>' },
  { name:"Bevel",         type:PowerPoint.GeometricShapeType.bevel,               svg:'<rect x="2" y="2" width="16" height="16" rx="1" fill="#777"/><rect x="5" y="5" width="10" height="10" fill="#555"/>' },
  { name:"Heart",         type:PowerPoint.GeometricShapeType.heart,               svg:'<path d="M10,16 C10,16 2,10 2,6 A4,4,0,0,1,10,5 A4,4,0,0,1,18,6 C18,10 10,16 10,16Z" fill="#555"/>' },
  { name:"Cross",         type:PowerPoint.GeometricShapeType.plus,                svg:'<rect x="7" y="2" width="6" height="16" fill="#555"/><rect x="2" y="7" width="16" height="6" fill="#555"/>' },
  { name:"Crescent",      type:PowerPoint.GeometricShapeType.moon,                svg:'<path d="M14,4 A8,8,0,1,0,14,16 A5,5,0,1,1,14,4Z" fill="#555"/>' },
  { name:"Frame",         type:PowerPoint.GeometricShapeType.frame,               svg:'<rect x="2" y="2" width="16" height="16" fill="none" stroke="#555" stroke-width="3"/>' },
  { name:"Donut",         type:PowerPoint.GeometricShapeType.donut,               svg:'<circle cx="10" cy="10" r="8" fill="#555"/><circle cx="10" cy="10" r="4" fill="white"/>' },
  { name:"LBracket",      type:PowerPoint.GeometricShapeType.leftBracket,         svg:'<path d="M13,2 Q6,2 6,10 Q6,18 13,18" fill="none" stroke="#555" stroke-width="2"/>' },
];
...
async function insertStandardShape(index) {
  ...
  await PowerPoint.run(async (context) => {
    ...
    const shape = slide.shapes.addGeometricShape(s.type, {
...
function addSVGToLibrary() {
  ...
  if (!code || !code.includes('<svg')) return showStatus("Paste valid SVG code first","err");
  ...
  showStatus(`✓ "${name}" added to library`);
}
...
function loadSVGLibraryUI() {
  ...
  grid.innerHTML = lib.map(item => `
    <div class="shape-item" onclick="insertSVGFromLibrary(${item.id})" title="${item.name}">
      <div style="width:28px;height:28px;overflow:hidden;display:flex;align-items:center;justify-content:center">
        ${item.code.replace(/width="[^"]*"/, 'width="28"').replace(/height="[^"]*"/, 'height="28"')}
      </div>
      <span>${item.name}</span>
...
function deleteSVGFromLib(id) {
  saveSVGLib(getSVGLib().filter(i=>i.id!==id));
  loadSVGLibraryUI();
}
...
async function createTable() {
  ...
  await PowerPoint.run(async (context) => {
    ...
    slide.load("width,height");
    await context.sync();
    const tw=slide.width*0.8, cw=colWI>0?colWI:tw/cols;
    ...
    shape.left=slide.width*0.1; shape.top=slide.height*0.15;
    await context.sync();
...
function saveTableStyle() {
  ...
  lib.push({id:Date.now(),name,
    rows:parseInt(document.getElementById('tblRows').value),cols:parseInt(document.getElementById('tblCols').value),
    rowH:parseFloat(document.getElementById('tblRowH').value),colW:parseFloat(document.getElementById('tblColW').value),
    headerBg:document.getElementById('tblHeaderBg').value,headerFg:document.getElementById('tblHeaderFg').value,
    headerSize:parseFloat(document.getElementById('tblHeaderSize').value),headerFont:document.getElementById('tblHeaderFont').value,
    ...
    row1:document.getElementById('tblRow1').value,row2:document.getElementById('tblRow2').value,
    bodyFg:document.getElementById('tblBodyFg').value,bodySize:parseFloat(document.getElementById('tblBodySize').value),
    bodyFont:document.getElementById('tblBodyFont').value,bodyAlign:document.getElementById('tblBodyAlign').value,
    borderColor:document.getElementById('tblBorder').value,borderW:parseFloat(document.getElementById('tblBorderW').value),
...
function loadTableStyle(item) {
  document.getElementById('tblRows').value=item.rows;document.getElementById('tblCols').value=item.cols;
  document.getElementById('tblRowH').value=item.rowH;document.getElementById('tblColW').value=item.colW||0;
  document.getElementById('tblHeaderBg').value=item.headerBg;document.getElementById('tblHeaderFg').value=item.headerFg;
  document.getElementById('tblHeaderSize').value=item.headerSize;document.getElementById('tblHeaderFont').value=item.headerFont;
  document.getElementById('tblHeaderAlign').value=item.headerAlign;
  document.getElementById('tblRow1').value=item.row1;document.getElementById('tblRow2').value=item.row2;
  document.getElementById('tblBodyFg').value=item.bodyFg;document.getElementById('tblBodySize').value=item.bodySize;
  document.getElementById('tblBodyFont').value=item.bodyFont;document.getElementById('tblBodyAlign').value=item.bodyAlign;
  document.getElementById('tblBorder').value=item.borderColor;document.getElementById('tblBorderW').value=item.borderW;
  document.getElementById('tblBorderWVal').textContent=item.borderW;document.getElementById('tblPadding').value=item.padding;
...
// ── FONT PICKER ───────────────────────────────────────
const FONTS=["Arial","Arial Black","Arial Narrow","Calibri","Calibri Light","Cambria","Century Gothic",
  "Comic Sans MS","Courier New","Franklin Gothic Medium","Garamond","Georgia","Gill Sans","Helvetica",
  "Impact","Lucida Console","Palatino","Rockwell","Tahoma","Times New Roman","Trebuchet MS","Verdana",
  "Baskerville","Didot","Montserrat","Oswald","Raleway","Roboto","Lato","Poppins","Inter","Futura"];
let allFonts=[...FONTS];
...
function renderFontDropdown(fonts) {
  document.getElementById('fontDropdown').innerHTML=fonts.map(f=>
    `<div class="font-option" style="font-family:'${f}'" onclick="selectFont('${f}')">${f}</div>`).join('');
}
function filterFonts(v){renderFontDropdown(allFonts.filter(f=>f.toLowerCase().includes(v.toLowerCase())));document.getElementById('fontDropdown').classList.add('open');}
function openFontDropdown(){renderFontDropdown(allFonts);document.getElementById('fontDropdown').classList.add('open');}
function selectFont(n){document.getElementById('txtFont').value=n;document.getElementById('fontDropdown').classList.remove('open');liveChar();}
document.addEventListener('click',e=>{if(!e.target.closest('.font-wrap'))document.getElementById('fontDropdown').classList.remove('open');});
  ...
  await PowerPoint.run(async (context) => {
    ...
    for(const s of sel.items){
      try{
        ...
        font.underline=underline?PowerPoint.ShapeFontUnderlineStyle.single:PowerPoint.ShapeFontUnderlineStyle.none;
      }catch(e){}
    }
    await context.sync();
...
async function applyPara(){
  ...
  const aMap={left:PowerPoint.ParagraphHorizontalAlignment.left,center:PowerPoint.ParagraphHorizontalAlignment.center,right:PowerPoint.ParagraphHorizontalAlignment.right,justify:PowerPoint.ParagraphHorizontalAlignment.justify};
  await PowerPoint.run(async(context)=>{
    const sel=context.presentation.getSelectedShapes();
    sel.load("items/type");await context.sync();
    for(const s of sel.items){try{s.textFrame.textRange.paragraphFormat.horizontalAlignment=aMap[align];}catch(e){}}
    await context.sync();showStatus("✓ Paragraph applied");
  }).catch(e=>showStatus(e.message,"err"));
}
...
function saveParaStyle(){
  ...
  if(!name) return;
  styles.push({id:Date.now(),name,fontName:document.getElementById('txtFont').value,style:document.getElementById('txtStyle').value,fontSize:parseFloat(document.getElementById('txtSize').value),fontColor:document.getElementById('txtColor').value,bold:document.getElementById('txtBold').classList.contains('on'),italic:document.getElementById('txtItalic').classList.contains('on'),underline:document.getElementById('txtUnderline').classList.contains('on'),strike:document.getElementById('txtStrike').classList.contains('on'),allCaps:document.getElementById('txtCaps').classList.contains('on'),superscript:document.getElementById('txtSuper').classList.contains('on'),subscript:document.getElementById('txtSub').classList.contains('on'),align:document.getElementById('txtAlign').value});
  saveParaStylesLib(styles);loadParaStyleLibraryUI();showStatus(`✓ "${name}" saved`);
}

function loadParaStyle(item){
  document.getElementById('txtFont').value=item.fontName;document.getElementById('txtStyle').value=item.style||'normal';
  document.getElementById('txtSize').value=item.fontSize;document.getElementById('txtColor').value=item.fontColor;
  document.getElementById('txtAlign').value=item.align;
  document.getElementById('txtBold').classList.toggle('on',!!item.bold);document.getElementById('txtItalic').classList.toggle('on',!!item.italic);
  document.getElementById('txtUnderline').classList.toggle('on',!!item.underline);document.getElementById('txtStrike').classList.toggle('on',!!item.strike);
  document.getElementById('txtCaps').classList.toggle('on',!!item.allCaps);document.getElementById('txtSuper').classList.toggle('on',!!item.superscript);
  document.getElementById('txtSub').classList.toggle('on',!!item.subscript);
  applyChar();applyPara();showStatus(`✓ Loaded "${item.name}"`);
}
...
function loadParaStyleLibraryUI(){
  ...
  el.innerHTML=styles.map(item=>`
    <div class="style-item" onclick="loadParaStyle(${JSON.stringify(item).replace(/"/g,'&quot;')})">
      <div><div style="font-family:'${item.fontName}';color:${item.fontColor};font-weight:${item.bold?'bold':'normal'};font-style:${item.italic?'italic':'normal'};font-size:13px">${item.name}</div>
      <div class="style-meta">${item.fontName} · ${item.fontSize}pt</div></div>
...
function addColor(){
  const hex=document.getElementById('newColor').value, name=document.getElementById('newColorName').value||hex;
...
function loadPaletteUI(){
  const p=getPalette();
  const mk=(fn,del)=>p.map(c=>`<div class="swatch" style="background:${c.hex}" title="${c.name}" onclick="${fn}('${c.hex}','${c.name}')">${del?`<span class="del" onclick="event.stopPropagation();deleteColor(${c.id})">×</span>`:''}</div>`).join('')||'<span style="font-size:11px;color:#aaa">No colors</span>';
...
function selectColor(hex,name){
  selectedColor=hex;
  document.getElementById('selectedColorPreview').innerHTML=`<span style="display:inline-block;width:12px;height:12px;background:${hex};border-radius:2px;margin-right:4px;vertical-align:middle"></span>${name}`;
}
...
async function bulkColorReplace(){
  ...
  await PowerPoint.run(async(context)=>{
    const slide=context.presentation.getSelectedSlides().getItemAt(0);
    slide.shapes.load("items/fill/foregroundColor,items/type");await context.sync();
    let count=0;
    for(const s of slide.shapes.items){try{if((s.fill.foregroundColor||'').replace('#','').toUpperCase()===fh){s.fill.setSolidColor(rh);count++;}}catch(e){}}
    await context.sync();showStatus(`✓ Replaced ${count} shape(s)`);
  }).catch(e=>showStatus(e.message,"err"));
}